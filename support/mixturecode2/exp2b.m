...
while delta < 1001
  ...
  common_file_prefix = strcat('./exp2b/data/delta_',delta_str,'/mvnorm_iter_');
  summary_file = strcat('./exp2b/summary/delta_',delta_str);
  summary = fopen(summary_file,'w');
  params_file = strcat('./exp2b/summary/delta_',delta_str,'_parameters');
  parameters = fopen(params_file,'w');
  data_folder = strcat('./exp2b/data/delta_',delta_str);
  success_rate = 0;
  inferred = [];
...
%    save(file_name,'sample','-ascii');
    ...
    mix_file = strcat('./exp2b/mixtures/delta_',delta_str,'/mvnorm_iter_',iter_str);
